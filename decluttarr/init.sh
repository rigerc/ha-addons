...
echo "[INFO] Initializing decluttarr Addon setup script..."
...
if command -v apt-get >/dev/null 2>&1; then
    ...
    apt-get install -yqq --no-install-recommends tar xz-utils libssl3 musl jq tzdata >/dev/null
    ...
    apk add --no-cache libssl3 musl jq tzdata >/dev/null
else
